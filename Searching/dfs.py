...
class BinarySearchTree:
  ...
  def recursive_dfs_post_order(self):
...
def traverse_post_order(node, list1):

  

  if node.left:
    traverse_post_order(node.left, list1)

  if node.right:
    traverse_post_order(node.right, list1)
...
print(tree.recursive_dfs_post_order())
